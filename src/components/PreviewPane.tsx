...
function findPreviews(obj: unknown, path = ""): PreviewItem[] {
  ...
  if (Array.isArray(obj)) {
    ...
    Object.entries(obj as JsonObject).forEach(([k, v]) => {
      ...
      if (typeof v === "string") {
        if (v.match(/^https?:\/\/.+\.(jpg|jpeg|png|gif|webp|svg|ico|avif|apng|bmp|tiff|webp)(\?.*)?$/i)) {
          result.push({ path: currentPath, value: v, type: "image", parentPath: path });
        } else if (v.match(/^https?:\/\/.+/i)) {
          result.push({ path: currentPath, value: v, type: "url", parentPath: path });
        }
...
function groupPreviews(items: PreviewItem[]): PreviewGroup[] {
...
function ImagePreview(props: { url: string; path: string }) {
  ...
  return (
    <div class="bg-[var(--bg-secondary)] rounded p-3">
      <div class="text-xs text-[var(--text-muted)] mb-2 truncate">{props.path}</div>
  ...
  return (
    <div class="bg-[var(--bg-secondary)] rounded p-3">
      ...
      >
        ...
        <Show when={!favicon()}>
          <span class="w-4 h-4 bg-[var(--bg-tertiary)] rounded text-xs flex items-center justify-center">🔗</span>
        </Show>
...
function PreviewGroup(props: { group: PreviewGroup }) {
  ...
  const imageCount = () => props.group.items.filter(i => i.type === "image").length;
  const urlCount = () => props.group.items.filter(i => i.type === "url").length;
...
export default function PreviewPane(props: Props) {
  ...
  const grouped = createMemo(() => groupPreviews(previews()));
  
  return (
    <div class="space-y-3 overflow-auto max-h-full">
      <Show when={previews().length === 0}>
        <p class="text-[var(--text-muted)] text-sm">No URLs or images found in the JSON</p>
      </Show>
      
      <For each={grouped()}>
        {(group) => (
          <PreviewGroup group={group} />
        )}
      </For>
    </div>
  );
