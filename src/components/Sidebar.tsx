...
function buildFileTree(files: R2File[]): TreeNode[] {
  ...
  files.sort((a, b) => a.key.localeCompare(b.key)).forEach((file) => {
...
function TreeItem(props: {
  ...
  const isSelected = () => props.node.type === "file" && props.selectedFile === props.node.path;
    ...
    <div class="select-none">
      ...
      >
        <Show when={props.node.type === "folder"}>
          ...
          >
            <svg class="w-3.5 h-3.5 text-[var(--text-muted)] shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" /></svg>
          </Show>
        </Show>
        <Show when={props.node.type === "folder"}>
          <Show
            when={props.node.expanded}
            fallback={
              <svg class="w-4 h-4 text-[var(--warning)] shrink-0" fill="currentColor" viewBox="0 0 24 24"><path d="M10 4H4c-1.11 0-2 .89-2 2v12c0 1.097.903 2 2 2h16c1.097 0 2-.903 2-2V8c0-1.097-.903-2-2-2h-8l-2-2z" /></svg>
            }
          >
            <svg class="w-4 h-4 text-[var(--warning)] shrink-0" fill="currentColor" viewBox="0 0 24 24"><path d="M20 6h-8l-2-2H4c-1.1 0-1.99.9-1.99 2L2 18c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V8c0-1.1-.9-2-2-2zm0 12H4V8h16v10z" /></svg>
          </Show>
        </Show>
...
export default function Sidebar(props: Props) {
  ...
  const filteredTree = createMemo(() => {
    ...
    return fileTree().map(filterNode).filter((n): n is TreeNode => n !== null);
  });
  ...
  return (
    <div class="h-full flex flex-col bg-[var(--bg-secondary)] border-r border-[var(--border-subtle)]">
      <div class="p-3 border-b border-[var(--border-subtle)]">
        <div class="relative">
          <svg class="absolute left-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-[var(--text-muted)]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
          <input
            type="text"
      ...
      <div class="flex-1 overflow-y-auto p-2">
        ...
        <Show when={loading()}>
          <div class="flex items-center justify-center gap-2 py-8">
            <svg class="w-4 h-4 animate-spin text-[var(--accent)]" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
        ...
        <Show when={!loading() && filteredTree().length === 0 && !error()}>
          <div class="text-center py-8">
            <svg class="mx-auto w-8 h-8 text-[var(--text-muted)] mb-2 opacity-50" fill="currentColor" viewBox="0 0 24 24"><path d="M14 2H6c-1.1 0-1.99.9-1.99 2L4 20c0 1.1.89 2 1.99 2H18c1.1 0 2-.9 2-2V8l-6-6zm2 16H8v-2h8v2zm0-4H8v-2h8v2zm-3-5V3.5L18.5 9H13z" /></svg>
